...
import {farmAddressOld} from '../core/config'
//连接钱包
import {provider_ether, farm_etherOld, claimLock_ether, tokenContract} from '../core/ethers'
...
export function fetchpoolInfo() {
  return {
    ...
    effects: async () => {
      const data = await farm_etherOld.poolInfo();
...
export function fetchInitRewardPercent() {
  return {
    ...
    effects: async () => {
      const blockNumber = await farm_etherOld.provider.getBlockNumber()
      const data = await farm_etherOld.getInitRewardPercent(blockNumber);
...
export const getPendingReward = async (pid,address) => {
  const data = await farm_etherOld.pendingReward(pid,address);
  // console.log('_pendingReward',data)
  return getTokenValue(data)
}
//apr
// export const getApr = async (pid) => {
//   const data = await farm_etherOld.poolApr(pid);
//   console.log('_Apr',data,data.toNumber()/10000*100)
//   return (data.toNumber()/10000*100).toFixed(2)
// }
//DailyReward
export const getDailyReward= async (pid) => {
  const data = await farm_etherOld.daylyReward(pid);
...
export function fetchHarvest() {
  return {
    ...
    effects: async () => {
      const data = await farm_etherOld.harvestAll();
...
export const fetchUserInfo = async (pid,address) => {
    const data = await farm_etherOld._userInfo(pid, address);
...
export const getTotalStaked = async (tokenAddress) => {
  ...
  let balance = await tokenContract.balanceOf(farmAddressOld)
...
export function fetchwithdraw(pid,amount,address,tokenAddress) {
  return {
    ...
    effects: async () => {
      let tokenContract = new ethers.Contract(tokenAddress, token, provider_ether)
      let d = await tokenContract.allowance(address, farmAddressOld)
      ...
      if (getTokenValue(d) == 0 || getTokenValue(d) < 1000000) {
        ...
        let tx = await signer.approve(farmAddressOld, toTokenValue(100000000))
        console.log('_fetchwithdraw-approve',tx)
        await tx.wait()
      }
      const signer = farm_etherOld.connect(provider_ether.getSigner())
      console.log('_fetchwithdraw-singer',signer)
      const tx = await signer.emergencyWithdraw(pid);
